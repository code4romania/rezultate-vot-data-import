        ...
        $ballots->each(function (string $ballot) use ($data, $turnouts) {
            ...
            Turnout::upsert(
                [
                    ...
                    'NullVotes' => $data->map(fn (array $county) => 0)->sum(),
                    'VotesByMail' => $data->map(fn (array $county) => 0)->sum(),
                    'ValidVotes' => $data->map(fn (array $county) => 0)->sum(),
                    'TotalSeats' => $data->map(fn (array $county) => 0)->sum(),
                    'Coefficient' => $data->map(fn (array $county) => 0)->sum(),
                    'Threshold' => $data->map(fn (array $county) => 0)->sum(),
                    'Circumscription' => $data->map(fn (array $county) => 0)->sum(),
                    'MinVotes' => $data->map(fn (array $county) => 0)->sum(),
                    'Mandates' => $data->map(fn (array $county) => 0)->sum(),
                    'CorrespondenceVotes' => $data->map(fn (array $county) => 0)->sum(),
                    'SpecialListsVotes' => $data->map(fn (array $county) => 0)->sum(),
